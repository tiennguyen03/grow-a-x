...
**Last updated:** 2026-06-19 by Nova (+ Claude) — **branch `nova/trim-redundant-visuals`** (folded into local `main`) trims now-redundant cell UI in favor of the Biosphere View (8E): **(1)** deleted the C-key **Matter Core panel** (`MatterConverterUI.luau`) and its `Main.client` wiring (Dust Value still shows in the inspect panel); **(2)** deleted the **orbiting planet cells** (`CellVisuals.client.luau`) — those cells are now represented only in the Life Vessel + Microscope; its evolution/cascade **celebration toasts moved into `BiosphereView`**; **(3)** the Life Vessel and Microscope now show **exactly one cell each (capped), never an inflated count** — vessel speckles = `min(cells, 120)`, microscope cells = `min(cells, 50)`. Pending in-Studio playtest.
...
| `src/client/BiosphereView.client.luau` | Dual-mode biosphere visualization + the evolution/cascade **celebration toasts**: Life Vessel orb (camera-relative; **one speckle per actual cell**, capped at 120) + Microscope `ViewportFrame` overlay (Brownian-motion cells; **exactly the real cell count**, capped at 50), toggled **M**/**Tab**. Derives data from `ConverterUpdate`; **no server change, never touches planet visuals**. Self-contained LocalScript, no Main wiring (Epic 3 / 8E) | ✅ Safe — self-contained LocalScript (Nova) |
