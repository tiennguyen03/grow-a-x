...
**Last updated:** 2026-06-19 by Nova (+ Claude) — **new branch `nova/organelle-upgrades`** adds the Epic 3 / Tier 1 **organelle upgrade path** (Sprint 8C): each Archaea Cell is upgraded organelle-by-organelle (Membrane → … → Nucleus) until it evolves into a **Eukaryotic Cell**; players own any number of cells, each with its own path; **all UI now lives in the planet inspect panel** (the Matter Core panel is obsolete). Pending in-Studio playtest. See the prior integration note below.
...
| 8C | Organelle upgrade path — upgrade each Archaea Cell organelle-by-organelle into a Eukaryotic Cell; unlimited cells; per-cell planet visuals + evolution celebration; **all UI in the inspect panel** | 🔶 On `nova/organelle-upgrades`; pending in-Studio playtest |
...
| `src/client/MatterConverterUI.luau` | **Matter Core** panel (toggle **C** / on-screen button) — read-only overview. **Now OBSOLETE/vestigial:** all interventions moved into the inspect panel (8C). Left in place but no interactive role; reads the back-compat `archaeaCount` field only | ✅ Safe — self-contained, but slated for removal |
...
| `src/client/CellVisuals.client.luau` | Per-cell, per-organelle planet visuals (cells orbit the planet; each organelle adds a cosmetic layer) + the "Eukaryotic Cell Unlocked!" green-pulse/particle celebration. Self-contained LocalScript, no Main wiring (Epic 3 / 8C) | ✅ Safe — self-contained LocalScript (Nova) |
