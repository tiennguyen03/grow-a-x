...
**Last updated:** 2026-06-19 by Nova (+ Claude) — **branch `nova/biosphere-view`** (folded into local `main`) adds Epic 3 **Sprint 8E: Biosphere View** — one new self-contained client script `src/client/BiosphereView.client.luau` with two togglable, data-sharing modes: a **Life Vessel** (glass orb floating near the camera; grows richer/glows/swirls with total cell count, heartbeat-pulses, tints to the dominant life form, bursts on create/evolve) and a **Microscope View** (a `ViewportFrame` "drop of water" overlay where cells swim with Brownian motion, scaling count + complexity + zoom-out with population). Toggle with **M** (or **Tab**) — open/close zooms in/out from the vessel. Pure client visualization of existing `ConverterUpdate` data: **no server change, never touches planet visuals**. Pending in-Studio playtest. See Sprint 8D below.
...
| `src/client/MatterConverterUI.luau` | **Matter Core** panel (toggle **C** only — the always-on top-right banner button was removed) — read-only overview. **Now OBSOLETE/vestigial:** all interventions moved into the inspect panel (8C). Shows balance / owned / **Dust Value ×N** (8D) + planet context; no interactive role | ✅ Safe — self-contained, but slated for removal |
...
| `src/client/CellVisuals.client.luau` | Per-cell, per-organelle planet visuals (cells orbit the planet; each organelle adds a cosmetic layer) + the "Eukaryotic Cell Unlocked!" green-pulse/particle celebration + the cascade celebration. Self-contained LocalScript, no Main wiring (Epic 3 / 8C–8D) | ✅ Safe — self-contained LocalScript (Nova) |
| `src/client/BiosphereView.client.luau` | Dual-mode biosphere visualization: Life Vessel orb (camera-relative, scales/glows/pulses with total cell count + dominant form) + Microscope `ViewportFrame` overlay (Brownian-motion cells), toggled **M**/**Tab**. Derives its data from `ConverterUpdate`; **no server change, never touches planet visuals**. Self-contained LocalScript, no Main wiring (Epic 3 / 8E) | ✅ Safe — self-contained LocalScript (Nova) |
