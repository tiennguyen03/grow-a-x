...
> **Polish pass (2026-06-19):** (1) **Terrain variation** — land/ice/cloud tiles now get deterministic per-tile size jitter + rectangular `width`/`length` + in-plane `roll` + color jitter (in `PlanetGenerator.generateSurface`; `PlayerPlanetService.buildTile` gained `width`/`length`/`roll` support), so the surface reads as organic patches instead of a uniform square grid. Same tile *count* (no perf change) and still seed-deterministic. (2) **Orbit distance** — `WorldConfig.PLANET_ORBIT_BASE_RADIUS` 340→**460** so the planet isn't cramped against the star (return-home / inspect / marker all derive from the live planet position, so nothing else needed updating).
