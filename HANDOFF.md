...
**Last updated:** 2026-06-19 by Nova (+ Claude) — **branch `nova/dynamic-pricing-1c`** (⚠️ **NOT in `main` yet** — lives on its own branch for review) adds Epic 3 **Sprint 1C: Multicellular Path + Dynamic Pricing**. New shared `Pricing.luau` (cell cost `15·(1+0.08n+0.001n²)`, upgrade cost `base·(1+0.1·bought)`) and `MulticellularData.luau` (the six Eukaryotic→Multicellular upgrades: Cell Adhesion, Signaling, Extracellular Matrix, Cell Cycle Coordination, Differentiation, Tissue Scaffolding). Server (`PlayerManager`) now charges the dynamic cell price, tracks `multicellularUpgrades`, and validates `PurchaseUpgrade` (Eukaryotic-gated, ordered, dynamic cost); `ConverterUpdate` carries `cellCost`/`multicellularUpgrades`/`nextUpgradeCost`. UI (`CellInterventionUI`) shows the live cell cost + a new **Multicellular Path** section (6 upgrades, live costs, Buy). The Microscope + Life Vessel layer a visual per upgrade (clustering → signaling pulses → matrix hull → coordinated breathing → differentiation tints → organized tissue / coordinated vessel pulse). **Note:** brief's `^1.2` cell formula didn't match its own example table, so I implemented the quadratic that does (one-line swap in `Pricing.luau`). Pending in-Studio playtest. See the previous microscope note below.

**Before that (in `main`):** **branch `nova/microscope-organelles`** made the Microscope cells accurate to the upgrade path and tidier: each microscope cell now **mirrors a real cell** (by index) and shows **only the organelles that cell has purchased** — a fresh Archaea Cell is a bare blob, organelles appear as they're bought (each cell rebuilds in place, new parts fade in). The cell layout was **redone for neatness** — removed the protruding flagella tail + oversized membrane; everything now sits inside the body (membrane = thin surface rim, cytoplasm fills the body, ribosomes a golden-angle scatter, plasmids a tidy ring). Organelles still also hide past per-organelle **zoom ceilings** (ribo/plasmid ≤20, nucleoid/mito ≤50, nucleus ≤60, membrane/cytoplasm ≤200). All in `BiosphereView.client.luau`; no server change. Pending in-Studio playtest. See the zoom-progression note below.

**Before that:** **branch `nova/microscope-zoom`** (folded into local `main`) gave the Microscope View a **dynamic zoom progression**: the viewport camera pulls back as the population grows (TweenService-animated log curve, ~9→140 studs), the swim region grows sub-linearly so cells pack denser as you zoom out, visible-cell cap raised 50→**200**, and new cells **animate as divisions** (1:1 with real cells — no phantoms).
...
| `src/shared/Pricing.luau` | Dynamic cost formulas shared by server + client: `cellCost(n)` (grows with cells owned) and `upgradeCost(base, bought)` (+10% per upgrade). Pure functions (Epic 3 / 1C) | ✅ Safe — pure data/functions |
| `src/shared/MulticellularData.luau` | Ordered Tier 1C path Eukaryotic → Multicellular Organism: the six upgrades (id/name/baseCost/visual/description) + helpers (`next`, `at`, `isComplete`, `COUNT`=6). Pure data (Epic 3 / 1C) | ✅ Safe — pure data, owned by progression work |
